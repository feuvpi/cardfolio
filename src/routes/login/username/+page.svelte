...
<script lang="ts">
	...
	async function confirmUsername() {
		...
		const querySnapshot = await getDocs(
			query(collection(db, 'usernames'), where('uuid', '==', $user!.uid))
		);
...
<AuthCheck>
	<h2 class="text-left w-2/5">Choose an username:</h2>
	...
	<div class="my-4">
		...
		{#if isValid && isAvailable && !loading && !isConfirmed}
			<button on:click={confirmUsername} class="btn btn-success"
